...
- [x] `python ramen_cve.py opml examples/sample.opml` produces a CSV and a Markdown report in the working directory without errors.
- [x] `python ramen_cve.py url <url>` extracts CVEs from a single page and produces the same output formats.
- [x] `python ramen_cve.py cve CVE-2021-44228 CVE-2021-26855` enriches the named CVEs and produces the same output formats.
- [x] `--start` and `--end` flags filter the CVE set by the active date mode.
- [x] `--date-mode {feed,disclosure,epss}` switches which date the filter applies to.
- [x] All four base buckets and the KEV override bucket are correctly assigned per the design.
- [x] CVSS and EPSS API calls are cached in SQLite with a 24-hour default TTL.
- [x] No secrets are hardcoded; the NVD API key loads from `.env`.
- [x] `ruff check .` passes with the project's configured rule set.
- [x] `pytest` passes against the test suite.
- [x] `README.md` covers install, key setup, three example invocations, and out-of-scope notes.
...
- Slice 0.1: bootstrap had todo/lessons at repo root not tasks/; created tasks/ dir and copied them; .gitignore/.env.example extracted from bundled zip.
- Slice 0.2: added Cache class stub alongside the pipeline stubs so ruff's F821 (undefined name) doesn't fire; long action strings split with implicit concatenation to stay under 100 chars.
- Slice 1.1: CVE regex needs (?!\d) negative lookahead to prevent 8-digit IDs matching as 7-digit.
- Slice 1.2: category is immediate parent outline text, not grandparent; fixed test expectation.
- Slice 3.1: requests must be top-level import (not inside function) for unittest.mock.patch to work; also patch time.sleep to keep NVD tests fast.
...
- [x] Create the directory layout from `claude.md`:
...
- [x] Open `ramen_cve.py` and add the top-of-file constants block:
...
- [x] Add the imports block (stdlib first, then third-party, alphabetical within each group).
- [x] Add a module docstring summarizing what the tool does in 3-5 lines.
- [x] Stub the dataclasses that will carry data through the pipeline:
...
- [x] Stub the four functions that will be the pipeline backbone, with docstrings only and `raise NotImplementedError` bodies:
...
- [x] Implement `extract_cves(text, source, first_seen)`:
...
- [x] Write unit tests in `tests/test_ramen_cve.py`:
...
- [x] Implement `parse_opml(path: Path) -> list[FeedEntry]`:
...
- [x] Add a tiny `examples/sample.opml` with 3-4 real feeds (Krebs RSS, CISA KEV RSS, Abuse.ch blog RSS, one nested under a category) for the smoke test in Phase 6.
- [x] Tests:
...
- [x] Implement a small `Cache` class wrapping `sqlite3.connect()`:
...
- [x] Implement `fetch_nvd(cve_id: str, cache: Cache, api_key: str | None) -> dict`:
...
- [x] Tests with mocked HTTP (use `unittest.mock.patch` on `requests.get`):
...
- [x] Implement `fetch_epss(cve_ids: list[str], cache: Cache, score_date: str | None = None) -> dict[str, dict]`:
...
- [x] Implement `enrich_cves(records: list[CveRecord], cache: Cache, api_key: str | None) -> list[EnrichedCve]`:
...
- [x] Implement `bucket_and_suggest(enriched, cvss_thr, epss_thr)`:
...
- [x] Tests covering every branch of the precedence:
...
- [x] Implement `filter_by_date(enriched, start, end, date_mode)`:
...
- [x] Implement `write_csv(enriched, path)`:
...
- [x] Implement `write_markdown(enriched, path, run_metadata)`:
...
- [x] Build the `argparse.ArgumentParser` with three subcommands: `opml`, `url`, `cve`.
...
- [x] Shared flags on all three subcommands: `--start`, `--end`, `--date-mode`, `--cvss-threshold`, `--epss-threshold`, `--out-dir`, `--format`, `--no-cache`, `--quiet`, `--verbose`.
- [x] Validation:
...
- [x] Logging configured per `--quiet`/`--verbose` to stderr.
- [x] Tests:
...
- [x] In `main()`, dispatch on the subcommand.
- [x] For `opml`:
...
- [x] Add `tests/fixtures/` with a few mocked NVD and EPSS payloads for stable CVEs (Log4Shell, ProxyLogon, etc.).
...
- [x] Run `ruff check . --fix` and review any auto-fixes.
- [x] Run `ruff format .` to normalize style.
- [x] Resolve any remaining lint warnings; suppress only with documented inline reasons.
...
- [x] Inspect the CSV and Markdown outputs by hand. Look for:
...
- [x] Save one of the runs into `examples/sample-output.csv` and `examples/sample-report.md`.
